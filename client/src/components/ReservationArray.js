            ...
            <div className='reservationsList'>
                ...
                <div>
                    ...
                    backendData.data.reservations.map(({reservationId, startDate, endDate, resourceId, ownerEmail, comments}, i) => (
                        <div key={i}>
                            <p key={i}>{i + 1}. {ownerEmail} booked from {startDate} until {endDate} using the {resourceId} resource and {comments !== null ? comments : 'nothing'} as comment</p>
                            <button className='delButton' key={"delButton"} onClick={() => handleClick(reservationId)}>Delete</button>
                            <button className='modButton' key={"modButton"} onClick={() => modifyReservation(reservationId, startDate, endDate, resourceId, ownerEmail, comments)} >Modify</button>
                        </div>
                    )))}
                </div>
            </div>
